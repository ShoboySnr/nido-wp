...
function pearl_save_settings()
{
    ...
    if (current_user_can('edit_theme_options')) {
        if (!empty($_POST)) {
            ...
            do_action('pearl_saving_settings');
            if ($updated) {
...
function pearl_get_theme_options_pairs($to)
{
    ...
    return apply_filters('pearl_theme_options_pairs_array', $sto);
}


add_filter('pearl_theme_options_pairs_array', function($to) {
    $disablePostTypeSinglePage = array('stm_staff', 'stm_testimonials');
    foreach($disablePostTypeSinglePage as $posttype) {
        if (isset($to[$posttype]['public'])) {
            $to[$posttype]['public'] = false;
        }
    }
    return $to;
});
